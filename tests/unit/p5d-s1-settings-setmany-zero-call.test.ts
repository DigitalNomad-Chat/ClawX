...
const channel = 'settings:setMany';
...
describe('P5-D-S1 settings:setMany legacy IPC zero-call evidence', () => {
  ...
  it('has no direct test calls for settings:setMany legacy channel', () => {
  ...
  it('has no ipcMain.handle registration for settings:setMany in main process', () => {
  ...
  it('has no settings:setMany fallback in host-api-client', () => {
    const client = readFileSync(join(repoRoot, 'src/lib/host-api-client.ts'), 'utf8');
    const regex = new RegExp(`invokeIpc\\(['"]${channel}['"]`, 'g');
    expect(regex.test(client)).toBe(false);
  });
});
