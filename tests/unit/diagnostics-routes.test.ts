...
const testOpenClawConfigDir = join(tmpdir(), 'clawdock-tests', 'diagnostics-routes-openclaw');
...
describe('handleDiagnosticsRoutes', () => {
  beforeEach(() => {
    ...
    readLogFileMock.mockResolvedValue('clawdock-log-tail');
  });
  ...
  it('returns diagnostics snapshot with channel view and tailed logs', async () => {
    ...
    expect(payload.clawdockLogTail).toBe('clawdock-log-tail');
