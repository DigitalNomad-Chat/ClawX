...
describe('buildOpenClawControlUiUrl', () => {
  it('uses the URL fragment for one-time token bootstrap', () => {
    expect(buildOpenClawControlUiUrl(18789, 'clawdock-test-token')).toBe(
      'http://127.0.0.1:18789/#token=clawdock-test-token',
    );
  });
  ...
  it('opens the Dreams view without moving the token out of the fragment', () => {
    expect(buildOpenClawControlUiUrl(18789, 'clawdock-test-token', { view: 'dreams' })).toBe(
      'http://127.0.0.1:18789/dreaming#token=clawdock-test-token',
    );
  });
