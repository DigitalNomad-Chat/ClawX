...
const channels = ['skill:getConfig', 'skill:getAllConfigs', 'skill:updateConfig'];

describe('P5-A skills config legacy IPC zero-call evidence', () => {
  ...
  it('has no direct test calls for skills config legacy channels', () => {
  ...
  it('has no ipcMain.handle registration for skills config channels in main process', () => {
  ...
  it('has no skills:* fallback entries in host-api-client', () => {
    const client = readFileSync(join(repoRoot, 'src/lib/host-api-client.ts'), 'utf8');
    const hits: string[] = [];
    for (const channel of channels) {
      const regex = new RegExp(`['"]${channel}['"]`, 'g');
      if (regex.test(client)) {
        hits.push(`src/lib/host-api-client.ts:${channel}`);
      }
    }
    expect(hits).toEqual([]);
  });
