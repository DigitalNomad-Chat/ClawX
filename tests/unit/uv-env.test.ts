...
beforeEach(() => {
  tempHome = mkdtempSync(path.join(tmpdir(), 'clawdock-uv-env-'));
  vi.resetModules();
  ...
  vi.doMock('electron', () => ({
    app: {
      ...
      getPath: vi.fn().mockReturnValue(path.join(tempHome, '.clawdock')),
...
describe('uv mirror environment', () => {
  it('writes a ClawDock-managed uv.toml under ~/.openclaw and exposes it via UV_CONFIG_FILE', async () => {
    const { getUvMirrorEnv } = await import('@electron/utils/uv-env');

    const env = await getUvMirrorEnv();

    const expectedPath = path.join(tempHome, '.openclaw', 'clawdock', 'uv.toml');
