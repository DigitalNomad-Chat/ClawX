...
vi.mock('@electron/utils/store', () => ({
  getSetting: vi.fn(async () => 'clawdock-route-token'),
}));
...
describe('GET /api/gateway/control-ui', () => {
  ...
  it('returns the default Control UI URL', async () => {
    ...
    expect(response.json).toMatchObject({
      success: true,
      url: 'http://127.0.0.1:19001/#token=clawdock-route-token',
      token: 'clawdock-route-token',
      port: 19001,
    });
  });
  ...
  it('returns the Dreams Control UI URL', async () => {
    ...
    expect(response.json).toMatchObject({
      success: true,
      url: 'http://127.0.0.1:19001/dreaming#token=clawdock-route-token',
      token: 'clawdock-route-token',
      port: 19001,
    });
  });
  ...
  it('falls back to the default Control UI URL for unknown views', async () => {
    ...
    expect(response.json.url).toBe('http://127.0.0.1:19001/#token=clawdock-route-token');
  });
});
