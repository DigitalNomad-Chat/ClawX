...
async function launchClawDockElectron(
  homeDir: string,
  userDataDir: string,
  ...
  return await electron.launch({
    ...
    env: {
      ...
      CLAWDOCK_E2E: '1',
      CLAWDOCK_USER_DATA_DIR: userDataDir,
      ...(options.skipSetup ? { CLAWDOCK_E2E_SKIP_SETUP: '1' } : {}),
      CLAWDOCK_PORT_CLAWDOCK_HOST_API: String(hostApiPort),
    },
    timeout: 90_000,
  });
}
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'clawdock-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'clawdock-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async (options?: LaunchElectronOptions) => await launchClawDockElectron(homeDir, userDataDir, options));
  },
