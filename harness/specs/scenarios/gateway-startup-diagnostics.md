...
Use this spec when ClawDock shows the Gateway as starting/running but UI data does not refresh, Dreams cannot load, or Gateway RPC calls time out after a restart.

ClawDock should prefer OpenClaw-native signals over stderr string matching:
...
- `gateway.ready`, `health`, and `presence` events should update ClawDock's main-process capability cache.
...
- **Handshake ready** only means ClawDock connected to the Gateway socket.
...
2. Read recent ClawDock logs:

```bash
tail -n 160 "$HOME/Library/Application Support/clawdock/logs/clawdock-$(date +%F).log"
```
...
```bash
pnpm exec openclaw gateway call system-presence >/tmp/clawdock-system-presence.json
pnpm exec openclaw gateway call health --params '{"probe":false}' >/tmp/clawdock-health.json
pnpm exec openclaw gateway call status >/tmp/clawdock-status.json
pnpm exec openclaw gateway call channels.status --params '{"probe":false}' >/tmp/clawdock-channels-status.json
pnpm exec openclaw gateway call doctor.memory.status >/tmp/clawdock-memory-status.json
pnpm exec openclaw gateway call doctor.memory.dreamDiary >/tmp/clawdock-dream-diary.json
```
...
```bash
sample <gateway-pid> 3 -mayDie >/tmp/clawdock-gateway.sample.txt
```
...
- Where the artifact will be written, usually `/tmp/clawdock-gateway.sample.txt`.
- What will be inspected and what will not be shared verbatim.

Do not proceed without explicit user agreement when:

- Sampling a process that is not clearly the ClawDock-owned Gateway child.
...
- Whether the finding points to ClawDock-owned prelaunch cleanup, OpenClaw runtime startup cost, active user work, or inconclusive data.
...
- Explain to users that restart cost is dominated by active Gateway work, not by ClawDock UI rendering.
...
- Logs no longer repeat stale runtime cache or escaped managed-skill symlink warnings for entries ClawDock can safely clean.
...
- Separate ClawDock-owned cleanup issues from OpenClaw runtime initialization cost.
