...
Browser fallback to `http://127.0.0.1:13210` is allowed only inside `src/lib/host-api.ts`, and only when `clawdock:allow-localhost-fallback` is explicitly enabled.
