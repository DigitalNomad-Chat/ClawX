...
function WindowsTitleBar() {
  ...
  useEffect(() => {
    // P4b-B1: host:invoke window.isMaximized with legacy window:* IPC fallback.
    void hostApi.window.isMaximized().then((val) => {
  ...
  const handleMaximize = () => {
    void hostApi.window.maximize().then(() =>
    ...
    >
      ...
      <div className="no-drag flex h-full">
        <button
          ...
          className="no-drag flex h-full w-11 items-center justify-center text-muted-foreground hover:bg-black/5 hover:text-foreground dark:hover:bg-white/10 transition-colors"
          title="Minimize"
        >
