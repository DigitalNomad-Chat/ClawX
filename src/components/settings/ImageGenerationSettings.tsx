...
/** Provider key reserved for the native OpenAI provider. Its API key is managed
 * elsewhere (not via this relay form), so the replace-key UI is suppressed. */
const OPENAI_NATIVE_PROVIDER_KEY = 'openai';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const existingRelayKeyUsable = snapshot?.openAiRelay?.apiKeyConfigured
        && snapshot.openAiRelay.providerKey !== OPENAI_NATIVE_PROVIDER_KEY;
  ...
  const handleTest = async () => {
    ...
    try {
      const result = await runImageGenerationTest({
        ...
        timeoutMs: snapshot?.config.timeoutMs ?? null,
      });
      if (result.success) {
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder={
                        snapshot?.openAiRelay?.apiKeyConfigured && snapshot.openAiRelay.providerKey !== OPENAI_NATIVE_PROVIDER_KEY
