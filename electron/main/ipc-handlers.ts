...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the ClawDock chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      ...
      const gatewayInput: Record<string, unknown> = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via ClawDock WebSocket chat events,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[clawdock-validate] validating provider type: ${providerType}`);
