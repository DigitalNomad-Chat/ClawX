...
// ---------------------------------------------------------------------------
//  Hall Rules (HALL.md) loading
// ---------------------------------------------------------------------------

const HALL_RULES_MAX_CHARS = 6000;
...
function resolveHallRulesGuidePath(): string | undefined {
  const explicit = process.env[HALL_RULES_OVERRIDE_ENV]?.trim();
  if (explicit) return explicit;
  const root = resolveProjectRoot();
  const candidate = join(root, HALL_RULES_DEFAULT_FILE);
  return existsSync(candidate) ? candidate : undefined;
}

interface HallRulesGuide {
  label: string;
  content: string;
  truncated: boolean;
}

function readHallRulesGuide(): HallRulesGuide | undefined {
...
function buildHallRulesPromptBlock(language: "zh" | "en"): string {
  ...
  return [
    isZh
      ? `## 协作风格指南（${guide.label}）`
      : `## Collaboration Style Guide (${guide.label})`,
    isZh
      ? "请在回复中遵循以下协作风格。这些规则只影响语气和协作方式，不要覆盖操作员的明确请求、当前负责人路由、队列执行顺序或硬性安全规则。"
...
export interface PromptBuildInput {
  ...
  /** Optional agent persona content from workspace files */
...
export function buildDispatchPrompt(input: PromptBuildInput): BuiltPrompt {
  ...
  const systemPrompt = buildSystemPrompt(mode, language, roleLabel, input.personaSummary);
...
function buildSystemPrompt(mode: DispatchMode, language: "zh" | "en", roleLabel: string, personaSummary?: string): string {
  const isZh = language === "zh";

  // Build persona prefix if available
  const personaPrefix = personaSummary
    ? isZh
      ? `以下是你的个人设定和背景信息，请在回复中体现：\n${personaSummary}\n\n`
      : `Here is your persona and background — reflect it in your replies:\n${personaSummary}\n\n`
    : "";

  // Load shared hall collaboration guide (HALL.md)
  const hallRulesBlock = buildHallRulesPromptBlock(language);

  const parts: string[] = [personaPrefix];
...
export function inferDiscussionDomain(taskCard: HallTaskCard): string {
  ...
  if (/\b(code|engineer|program|develop|api|backend|frontend|bug|fix|refactor|test|deploy|db|database|server|cloud|infra|devops|git|commit|pr|ci|cd)\b/.test(text)) return "engineering";
  if (/\b(design|creative|copy|content|brand|video|thumbnail|hook|script|storyboard|marketing|ad|campaign|media|visual|ui|ux|figma|sketch)\b/.test(text)) return "creative";
  if (/\b(analys|research|data|survey|report|metric|kpi|insight|trend|benchmark|competitor|market|ab|experiment|segment|funnel|conversion)\b/.test(text)) return "analysis";
  if (/\b(product|feature|requirement|spec|prd|roadmap|user story|mvp|launch|release|version|epic|backlog)\b/.test(text)) return "product";
  if (/\b(ops|operation|workflow|process|automation|schedule|logistics|supply|inventory|support|ticket|sla|oncall|pager|incident)\b/.test(text)) return "operations";
  if (/\b(legal|law|contract|compliance|regulation|gdpr|privacy|license|liability|policy|terms|clause)\b/.test(text)) return "legal";
  if (/\b(medical|health|patient|diagnosis|drug|clinical|therapy|hospital|doctor|treatment|symptom)\b/.test(text)) return "medical";
  if (/\b(education|course|curriculum|student|teacher|learn|training|certification|exam|lesson|mooc)\b/.test(text)) return "education";
  if (/\b(finance|budget|revenue|cost|profit|invest|fund|accounting|tax|invoice|payment|pricing|billing|subscription)\b/.test(text)) return "finance";
  if (/\b(security|vulnerability|pentest|exploit|cve|xss|csrf|injection|auth|oauth|jwt|encryption|hash)\b/.test(text)) return "security";
  return "general";
}
