...
export function enforceConcreteDeliverable(
  ...
  // Discussion mode (non-direct) does not require concrete deliverables
  if (mode === "discussion" && operatorIntent?.type !== "direct_ask") {
    return { content: visibleContent };
  }

  const currentTask = mode === "discussion"
    ? (operatorIntent?.text ?? "")
    : (task ?? "");
  ...
  if (mode === "discussion" && operatorIntent?.type === "direct_ask" && deliverableKind !== "generic") {
