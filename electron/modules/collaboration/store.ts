...
export async function updateTaskCard(
  ...
  // Emit structured-update event if proposal/decision/summary fields changed
  if (input.proposal !== undefined || input.decision !== undefined || input.latestSummary !== undefined) {
    publishCollabEvent({
      type: "invalidate",
      hallId: taskCard.hallId,
      taskCardId: taskCard.taskCardId,
...
// ---------------------------------------------------------------------------
//  Discussion Cycle operations
// ---------------------------------------------------------------------------

/**
 * Record that a participant has responded in the current discussion cycle.
 * If all expected participants have responded, automatically close the cycle.
 */
export async function recordDiscussionResponse(
  taskCardId: string,
  participantId: string,
): Promise<{ cycleCompleted: boolean; taskCard: HallTaskCard }> {
  const { markDiscussionSpeakerComplete, closeDiscussionCycle } = await import("./speaker-policy");
  ...
  let updated = markDiscussionSpeakerComplete(taskCard, participantId);
  const cycle = updated.discussionCycle;
  let cycleCompleted = false;

  // Check if all expected participants have responded
  if (cycle && cycle.expectedParticipantIds.every((id) => cycle.completedParticipantIds.includes(id))) {
    updated = closeDiscussionCycle(updated);
    cycleCompleted = true;
  }

  const saved = await updateTaskCard(taskCardId, {
    discussionCycle: updated.discussionCycle,
  });

  // Notify frontend when discussion cycle closes
  if (cycleCompleted) {
    publishCollabEvent({
      type: "invalidate",
      hallId: saved.hallId,
      taskCardId,
      reason: "discussion_cycle_close",
      payload: {
        discussionCycle: saved.discussionCycle,
      },
    });
  }

  return { cycleCompleted, taskCard: saved };
}
...
export async function releaseExecutionLock(
  ...
  const now = new Date().toISOString();
  lock.releasedAt = now;
  lock.releasedReason = releasedReason ?? "released";
  taskCard.updatedAt = now;
  store.updatedAt = now;

  await getTaskCardStore().write(store);

  // Notify frontend about lock release
  publishCollabEvent({
    type: "invalidate",
    hallId: taskCard.hallId,
    taskCardId,
    reason: "lock_released",
    payload: { lockId: lock.lockId, participantId: lock.participantId, releasedReason: releasedReason ?? "released" },
  });
}
...
/**
 * List artifacts for a task card.
 */
export async function listArtifacts(taskCardId: string): Promise<import("./types").TaskArtifact[]> {
  const store = await loadTaskCardStore();
  const taskCard = getTaskCard(store, taskCardId);
  if (!taskCard) throw new Error(`Task card '${taskCardId}' not found`);
  return taskCard.artifactRefs ?? [];
}

/**
 * Add a new artifact to a task card.
 */
export async function addArtifact(
  taskCardId: string,
  input: AddArtifactInput,
): Promise<import("./types").TaskArtifact> {
  const store = await loadTaskCardStore();
  const taskCard = getTaskCard(store, taskCardId);
  if (!taskCard) throw new Error(`Task card '${taskCardId}' not found`);

  const artifactId = `artifact-${stableHash(input.location).toString(16)}`;
  const artifact: import("./types").TaskArtifact = {
    artifactId,
    type: input.type,
    label: input.label.trim() || inferArtifactLabel(input.location),
    location: input.location.trim(),
  };

  const existing = taskCard.artifactRefs ?? [];
  const seen = new Set(existing.map((a) => a.location.toLowerCase()));
  if (seen.has(artifact.location.toLowerCase())) {
    throw new Error(`Artifact with location '${artifact.location}' already exists`);
  }

  const merged = [...existing, artifact];

  const updated = await updateTaskCard(taskCardId, { artifactRefs: merged });
  return artifact;
}

/**
 * Remove an artifact from a task card.
 */
export async function removeArtifact(
  ...
  const existing = taskCard.artifactRefs ?? [];
  const filtered = existing.filter((a) => a.artifactId !== artifactId);

  if (filtered.length === existing.length) {
    throw new Error(`Artifact '${artifactId}' not found on task card '${taskCardId}'`);
  }

  await updateTaskCard(taskCardId, { artifactRefs: filtered });
}
