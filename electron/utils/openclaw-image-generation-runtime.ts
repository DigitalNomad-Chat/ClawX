...
/**
 * Maximum allowed size (in bytes) for a single generated image persisted via
 * `saveMediaBuffer`.  Generated images are produced by the relay and are
 * expected to be modest PNG/JPEG payloads; capping at 25 MiB prevents an
 * errant relay from filling the media store with arbitrarily large buffers.
 * Exported so tests and callers can reference the same bound.
 */
