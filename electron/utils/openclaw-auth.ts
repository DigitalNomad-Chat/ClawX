...
export async function setOpenClawDefaultModel(
  ...
  return withConfigLock(async () => {
    ...
    } else if (provider === OPENAI_CODEX_RUNTIME_PROVIDER_KEY) {
 ...
 * Used by ClawDock to seed the provider store when it's empty but providers are
 ...
 * which means conversations disappear after roughly one day.  ClawDock sets
 ...
 * OpenClaw/ClawDock versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // for ClawDock to properly integrate with its updated tool system.
    ...
    // ClawDock is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for ClawDock desktop');
    }
    ...
    // ── anthropic-messages maxTokens self-heal ──────────────────────
    // OpenClaw 3.30+ requires every anthropic-messages provider/model to
    // specify a positive maxTokens. Existing configs may have zero or
    // missing values, causing runtime validation failures. Repair them
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // ClawDock-critical bundled plugins, active provider plugins, and explicitly
