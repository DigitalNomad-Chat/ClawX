...
function buildClawDockUvConfigToml(): string {
  return [
    '# This file is managed by ClawDock.',
    '# It lets uv use ClawDock-selected mirrors even when host exec filters UV_INDEX_URL.',
...
export function getClawDockUvConfigFilePath(): string {
  return path.join(getOpenClawConfigDir(), 'clawdock', 'uv.toml');
}

function ensureClawDockUvConfigFile(): string | null {
  const filePath = getClawDockUvConfigFilePath();
  const content = buildClawDockUvConfigToml();
  ...
  try {
    ...
    logger.warn('Failed to write ClawDock uv config file:', err);
    return null;
  }
}
...
export async function getUvMirrorEnv(): Promise<Record<string, string>> {
  ...
  const uvConfigFile = ensureClawDockUvConfigFile();
  return uvConfigFile
