...
/**
 * Input for the atomic image-generation settings write.
 *
 * `openAiRelay` mirrors the relay-facing knobs (enable/disable, base URL, key,
 * model) and `modelRef` is the full `provider/model` primary to persist into
 * `agents.defaults.imageGenerationModel.primary` (pass `null` to clear it).
 */
export interface ImageGenerationSettingsInput {
  ...
  /**
   * Image-generation timeout in ms. `undefined` preserves any existing value
   * already stored in the config; `null` clears it.
   */
  timeoutMs?: number | null;
}

/**
 * Apply the full image-generation settings in a single atomic write.
 *
 * The PUT /api/media/image-generation handler previously called
 * `applyOpenAiImageRelaySettings` and `setImageGenerationConfig` back-to-back,
 * each of which took the config lock independently. That left a window where a
 * concurrent reader (or another writer) could observe a half-applied state.
 *
 * This function performs the relay upsert + plugin registration + key save and
 * the `agents.defaults.imageGenerationModel.primary` write inside ONE
 * `withConfigLock` critical section. `withConfigLock` is reentrant, so the
 * inner calls into `syncOpenAiCompatibleImageRelay` / `setImageGenerationConfig`
 * pass through without re-acquiring (and without deadlocking).
 */
export async function applyImageGenerationSettings(
  settings: ImageGenerationSettingsInput,
): Promise<void> {
  await withConfigLock(async () => {
    if (settings.openAiRelay) {
      await applyOpenAiImageRelaySettings({
        enabled: settings.openAiRelay.enabled,
        baseUrl: settings.openAiRelay.baseUrl,
        apiKey: settings.openAiRelay.apiKey,
        model: settings.openAiRelay.model,
      });
    }
    // Preserve the existing timeout unless the caller explicitly overrides
    // (number) or clears (null) it. Reading inside the lock guarantees we
    // observe the value written by the relay step above.
    const existing = settings.timeoutMs === undefined
      ? (await readImageGenerationConfig()).timeoutMs
      : settings.timeoutMs;
    await setImageGenerationConfig({
      primary: settings.modelRef,
      fallbacks: [],
      timeoutMs: existing,
    });
  });
