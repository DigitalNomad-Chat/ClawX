...
interface SyncProxyOptions {
   ...
   * currently disabled in ClawDock settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync ClawDock global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because ClawDock proxy is disabled and preserve mode is enabled');
      return;
    }
