...
/**
 * Normalize an OpenClaw Gateway agent stream payload into the shared
 * ChatRuntimeEvent contract. Returns null for unknown/malformed input.
 *
 * Important: lifecycle phase=end is intentionally NOT mapped to run.ended.
 * ClawDock treats non-terminal phase=end as a mid-run hint, not run completion.
 */
export function normalizeGatewayChatRuntimeEvent(payload: unknown): ChatRuntimeEvent | null {
  ...
  if (stream === 'lifecycle') {
    ...
    // phase=end and other non-terminal phases intentionally return null
    return null;
  }
