...
export function dispatchProtocolEvent(
  ...
  switch (event) {
    ...
    case 'chat':
      ...
      break;
    case 'agent': {
      // Dual-emit: normalized runtime event (for future M1+ consumers) +
      // legacy notification (current ClawDock chat path). Do not remove the
      // notification path; phase=end non-terminal semantics live there.
