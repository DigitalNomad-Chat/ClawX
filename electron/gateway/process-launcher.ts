...
(function () {
  ...
  if (globalThis.__clawdockFetchPatched) return;
  globalThis.__clawdockFetchPatched = true;

  globalThis.fetch = function clawdockFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      flat['X-OpenRouter-Title'] = 'ClawDock';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__clawdockPatched) {
        cp.__clawdockPatched = true;
...
export async function launchGatewayProcess(options: {
  ...
  // which causes:
  //   - cross-machine name collisions when multiple OpenClaw/ClawDock peers
  //     share a LAN (each falls back to "<name> (OpenClaw) (2)")
  //   - self-collisions on multi-homed hosts (Wi-Fi + Tailscale + utun ...)
  //   - "ghost" record collisions after an unclean ClawDock exit, because
  //     SIGKILL prevents ciao from emitting the mDNS goodbye record.
  //
  // ClawDock has no UI for LAN gateway discovery today, so the advertiser is
