...
import { ensureClawDockContext } from '../../utils/openclaw-workspace';
    ...
    if (pid) {
      ...
      try {
        if (process.platform === 'darwin' || process.platform === 'linux') {
          // MUST use -sTCP:LISTEN. Otherwise lsof returns the client process (ClawDock itself)
...
export async function handleAgentRoutes(
  ...
  if (url.pathname === '/api/agents' && req.method === 'POST') {
    try {
      ...
      // Ensure newly provisioned workspaces get ClawDock context merge/cleanup
      // even when gateway status events do not fire (e.g. in-process reload).
      void ensureClawDockContext({ waitForAllConfiguredWorkspaces: true }).catch((err) => {
        console.warn('[agents] Failed to ensure ClawDock context after agent creation:', err);
      });
