...
export function getListenPort(env: Record<string, string | undefined> = process.env): number {
  const port = parseInt(env.PORT || '8648', 10)
  return Number.isNaN(port) ? 8648 : port
}
...
export const config = {
  ...
  dataDir: process.env.HERMES_DATA_DIR || resolve(__dirname, '..', 'data'),
