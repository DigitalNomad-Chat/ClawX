...
import { basename, delimiter, dirname, join, win32 as win32Path } from "node:path";
...
function ensureCleanOpenClawConfig(): void {
  ...
  try {
    ...
    // Remove plugins section to avoid stale plugin validation errors that
    // block CLI execution when the Desktop app rewrites the config.
    delete data.plugins;
    writeFileSync(CLEAN_CONFIG_PATH, `${JSON.stringify(data, null, 2)}\n`, "utf8");
  } catch {
    // Best-effort: if the clean config cannot be created, fall back to
    // the original config and let the caller handle any CLI errors.
  }
}
...
export function buildOpenClawCommandEnv(
  ...
  const baseEnv = platformName !== "win32" ? { ...env } : buildWindowsOpenClawCommandEnv(env, platformName);

  // Control Center treats OPENCLAW_HOME as the data directory (e.g. ~/.openclaw),
  // but the official OpenClaw CLI treats it as the parent directory and appends
  // .openclaw internally. Correct the value before passing it to the CLI so the
  // CLI resolves the state dir as ~/.openclaw instead of ~/.openclaw/.openclaw.
  ...
  // Use a sanitized config copy to bypass stale plugin validation errors.
  ensureCleanOpenClawConfig();
  if (existsSync(CLEAN_CONFIG_PATH) && !env.OPENCLAW_CONFIG_PATH?.trim()) {
...
function buildWindowsOpenClawCommandEnv(
