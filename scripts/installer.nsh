; ClawDock Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting ClawDock runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers ClawDock.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if ClawDock.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old ClawDock was installed
  ...
  ; "Can't modify ClawDock's files" retry loop -> "ClawDock 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "ClawDock 无法关闭" dialog.
...
; "ClawDock 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with ClawDock's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove ClawDock application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\clawdock (local app data)$\r$\n  • AppData\Roaming\clawdock (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering ClawDock processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\clawdock"
    RMDir /r "$APPDATA\clawdock"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\clawdock
    IfFileExists "$LOCALAPPDATA\clawdock\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\clawdock"
      IfFileExists "$LOCALAPPDATA\clawdock\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\clawdock"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\clawdock
    IfFileExists "$APPDATA\clawdock\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\clawdock"
      IfFileExists "$APPDATA\clawdock\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\clawdock"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\clawdock\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\clawdock"
    IfFileExists "$APPDATA\clawdock\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\clawdock"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\clawdock"
    RMDir /r "$R3\AppData\Roaming\clawdock"
