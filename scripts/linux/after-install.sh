...
# Post-installation script for ClawDock on Linux
...
# Create symbolic link for ClawDock app binary
if [ -x /opt/ClawDock/clawdock ]; then
    ln -sf /opt/ClawDock/clawdock /usr/local/bin/clawdock 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ClawDock/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ClawDock/chrome-sandbox' || true
else
    chmod 0755 '/opt/ClawDock/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ClawDock/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/clawdock'
...
echo "ClawDock has been installed successfully."
